...
import mvpPublicationReadiness from "@/data/mvp-publication-readiness.json";
...
const batchStatusLabels = {
  ready_as_source_archive: "Kaynaklı arşiv hazır",
  needs_source_completion: "Kaynak tamamlanmalı",
  waiting_market_data: "Resmi veri bekleniyor"
};

const publicationEventStatusLabels = {
  source_archive_with_data_pending_notice: "Kaynaklı arşiv",
  hold: "Yayın bekliyor"
};
...
function getPublicationEventStatusLabel(status) {
  return publicationEventStatusLabels[status] ?? status;
}

function getCalculationStatusLabel(status) {
...
export function getEventPublicationState(event) {
  const readinessEvent = mvpPublicationReadiness.events.find((item) => item.slug === event.slug);

  if (!readinessEvent) {
    return {
      slug: event.slug,
      ticker: event.ticker,
      publicMode: event.dataStatus === "sample" ? "sample_demo" : "hold",
      publicModeLabel: event.dataStatus === "sample" ? "Örnek arayüz" : "Yayın bekliyor",
      calculationStatus: event.dataStatus === "sample" ? "sample" : "waiting_market_data",
      calculationStatusLabel: event.dataStatus === "sample" ? "Örnek veri" : "Resmi veri bekleniyor",
      verificationStatus: event.verificationStatus,
      isVerified: event.verificationStatus === "verified",
      isFirstSourceBatch: false,
      decision:
        event.dataStatus === "sample"
          ? "Bu kayıt arayüz örneğidir; gerçek piyasa tepkisi olarak sunulmaz."
          : "Kaynak ve piyasa verisi tamamlanana kadar kayıt yayına alınmaz."
    };
  }

  return {
    ...readinessEvent,
    publicModeLabel: getPublicationEventStatusLabel(readinessEvent.publicMode),
    calculationStatusLabel: getCalculationStatusLabel(readinessEvent.calculationStatus),
    isVerified: readinessEvent.verificationStatus === "verified",
    isFirstSourceBatch: mvpPublicationReadiness.recommendedBatches[0]?.tickers?.includes(readinessEvent.ticker) ?? false,
    decision:
      readinessEvent.calculationStatus === "calculated"
        ? "Hesaplama tamamlandı; verified statüsü için editör kontrolü gerekir."
        : "Kaynak bulundu, fakat fiyat tepkisi resmi piyasa verisi gelmeden doğrulanmış sayılmaz."
  };
}
...
export function getDataOperationSnapshot() {
  ...
  return {
    ...
    mvpPublication: {
      asOfDate: mvpPublicationReadiness.asOfDate,
      status: mvpPublicationReadiness.status,
      principle: mvpPublicationReadiness.principle,
      summary: mvpPublicationReadiness.summary,
      publicationGates: mvpPublicationReadiness.publicationGates,
      recommendedBatches: mvpPublicationReadiness.recommendedBatches.map((batch) => ({
        ...batch,
